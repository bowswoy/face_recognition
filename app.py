...
def generate_dataset(pid):
    ...
    while True:
        ...
        if face_cropped(img) is not None:
            ...
            cv2.putText(face, "img_" + str(count_img), (10, 190), cv2.FONT_HERSHEY_DUPLEX, 0.5, (0, 255, 0), 0)
            ## insert to db
            cursor.execute("""INSERT INTO `img_dataset`(`img_id`, `img_person`) VALUES('{}', '{}')""".format(img_id, pid))
            conn.commit()
            frame = cv2.imencode('.jpeg', face)[1].tobytes()
    ...
    def draw_boundary(img, classifier, scaleFactor, minNeighbors, color, _, clf):
        ...
        for (x, y, w, h) in features:
            ...
            if s is not None:
                s = ''.join(s)
            if confidence > 70:
                cv2.putText(img, s + " (" + str(confidence) + "%)", (x, y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 2, cv2.LINE_AA)
            else:
                cv2.putText(img, "UNKNOWN", (x, y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 1, cv2.LINE_AA)
            coords = [x, y, w, h]
        return coords
